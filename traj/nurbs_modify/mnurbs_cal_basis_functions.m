...
function [Niku] = mnurbs_cal_basis_functions(u,i,k,u_vec)  
...
%     if(k==0)
%         if(u_vec(i)<=u && u<u_vec(i+1))%注意1=u(i)<=t<u(i+1)=1时的情况,这里要用t<=u(i+1);
%             Niku=1;
%             return;
%         else
%             Niku=0;
...
%         end
%     else
%         if(u_vec(i+k)-u_vec(i)==0)
%             alpha=0;
%         else
%             alpha=(u-u_vec(i))/(u_vec(i+k)-u_vec(i));
%         end
%         if(u_vec(i+k+1)-u_vec(i+1)==0)
%              beta=0;
%         else
%             beta=(u_vec(i+k+1)-u)/(u_vec(i+k+1)-u_vec(i+1));
%         end
%     end
%     Niku=alpha*nurbs_cal_basis_functions(u,i,k-1,u_vec)+beta*nurbs_cal_basis_functions(u,i+1,k-1,u_vec);
