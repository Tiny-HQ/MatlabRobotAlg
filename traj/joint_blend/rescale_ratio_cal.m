...
%缩放模块; zoom module;
%S是距离，不是位移; S is the distance, not the displacement;

function [rescale_ratio,s,v,a,j] = rescale_ratio_cal(S,V,A,J)


    axis_no = length(S);
    MAX_SEG = 7;%七段加减速的数组最后一段; the last segment of the array of seven acceleration and deceleration;
    main_axis = -1;%主轴编号 main axes number
    main_axis_time = 0.0;%主轴时间; main axes time;
