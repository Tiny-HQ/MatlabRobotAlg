...
%% 可以称之为解析法，但实际上由于加速度的原因，一般不会采用，高速运行起来，驱动器啥的都会报警,但是理论上是行的通的;
...
Ts = 0.004;
J = [0 -30 60 20 70 -100];    %关节角度;
Vel = [10 55 20 30 10];      %关节速度;
Acc = [100 50 200 300 100]; 
Jerk = [500 250 1000 1500 500];
n = 5;                      %点的个数减一;


%% 求每一段匀速所能达到的速度，有时候可能设置的速度到不到; Find the speed that can be achieved by each section of constant speed, sometimes the speed may not be set;
...
%% 求blend段多项式参数系数; Find the polynomial parameter coefficients of the blend segment;
P_acc = zeros(1,n);
P_dec = zeros(1,n);
a = zeros(1,n-1);b = zeros(1,n-1);c = zeros(1,n-1);d = zeros(1,n-1);e = zeros(1,n-1);f = zeros(1,n-1);  %系数;
for i = 1:n
...
%% 求各个阶段点位生成;Generate points at each stage;
...
%% 把所有的轨迹整合一起; %% Integrate all the trajectories together;
