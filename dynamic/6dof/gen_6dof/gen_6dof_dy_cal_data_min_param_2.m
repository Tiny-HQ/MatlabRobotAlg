...
function tao = gen_6dof_dy_cal_data_min_param_2(q,dq,ddq)
...
%     tao = U_r*fi;
    ...
    YY3 = Ii33yy+a4^2*M4;M3 = m3+M4;
    MX3 = m3*Pc333(1)+a4*M4;MY3 = m3*Pc333(2)+(MZ4+d4*M4);MZ3 = m3*Pc333(3);
    XX3 = Ii33xx-YY3+YY4+2*d4*MZ4+d4^2*M4;XY3 = Ii33xy-a4*(MZ4+d4*M4);XZ3 = Ii33xz;YZ3 = Ii33yz;ZZ3 = Ii33zz+YY4+2*d4*MZ4+(d4^2+a4^2)*M4;
    
    YY2 = Ii22yy+(a3^2+d3^2)*M3+YY3+2*d3*MZ3;M2 = m2+m3+m4+m5+m6;
    MX2 = m2*Pc222(1)+a3*M3;MY2 = m2*Pc222(2);MZ2 = m2*Pc222(3)+MZ3;
    XX2 = Ii22xx-YY2+YY3+2*d3*MZ3+d3^2*M3; XY2 = Ii22xy;XZ2 = Ii22xz-a3*(MZ3+d3*M3);YZ2 = Ii22yz;ZZ2 = Ii22zz+a3^2*M3;  
     
    ZZ1 = Ii11zz+YY2+a2^2*M2;%% 
    fi1_min_param = [ZZ1]';
