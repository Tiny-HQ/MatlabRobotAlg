...
tic;
s = mnurbs_simpson_cal_length(u(1),u(length(u)),u_vec,k,P(:,1),w',P(:,2),w',P(:,3),w')
toc;
...
for j = 1:length(u)
    ...
    [Cu(j,1),dCu(j,1),ddCu(j,1)] = mnurbs_cal_ddcu_derivative(u(j),k,u_vec,P(:,1),w');
    [Cu(j,2),dCu(j,2),ddCu(j,2)] = mnurbs_cal_ddcu_derivative(u(j),k,u_vec,P(:,2),w');
    [Cu(j,3),dCu(j,3),ddCu(j,3)] = mnurbs_cal_ddcu_derivative(u(j),k,u_vec,P(:,3),w');
...
A_max = 100;
J_max = 500;
[k] = mnurbs_cal_candidate_split_point(100,1000,5000,k_c);
...
plot3(P(:,2),P(:,3),P(:,1));hold on;
plot3(P(1,2),P(1,3),P(1,1),'o');
