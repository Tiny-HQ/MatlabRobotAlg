...
px = mnurbs_cal_cu(u(20),k,u_vec,P(:,1),Wx);

for j = 1:length(u)

    %i = floor(u(j)/Ts);%determin index of knot vector;
    Cu(j,1) = mnurbs_cal_cu(u(j),k,u_vec,P(:,1),Wx);
    Cu(j,2) = mnurbs_cal_cu(u(j),k,u_vec,P(:,2),Wy);
    Cu(j,3) = mnurbs_cal_cu(u(j),k,u_vec,P(:,3),Wz);
end


for i = 2:m
    
    Cu_v(i,1) = mnurbs_cal_cu(u_vec(i+k),k,u_vec,P(:,1),Wx);
    Cu_v(i,2) = mnurbs_cal_cu(u_vec(i+k),k,u_vec,P(:,2),Wy);
    Cu_v(i,3) = mnurbs_cal_cu(u_vec(i+k),k,u_vec,P(:,3),Wz);

end
Cu(1,1) = P(1,1);Cu(1,2) = P(1,2);Cu(1,3) = P(1,3);
Cu(length(u),1) = P(m,1);Cu(length(u),2) = P(m,2);Cu(length(u),3) = P(m,3);hold on;
plot3(Cu(:,1),Cu(:,2),Cu(:,3));hold on;
plot3(P(:,1),P(:,2),P(:,3))

Cu_v(1,1) = P(1,1);Cu_v(1,2) = P(1,2);Cu_v(1,3) = P(1,3);
hold on;
plot3(Cu_v(:,1),Cu_v(:,2),Cu_v(:,3),'*');
