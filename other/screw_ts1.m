...
%六自由度旋量正解;forward solution of six-degree-of-freedom screw;
%应该是正确的，跟dh计算的结果相同，二轴的角度相差90，dh中二轴90度对应这里的0度; It should be correct, the same as the result of DH calculation, the angle difference of the two axes is 90, and the 90 degrees of the two axes in DH correspond to 0 degrees here;
...
w_r(1,1) = 0;w_r(1,2) = -A3;w_r(1,3) = A2;%w的反对称矩阵; w's antisymmetric matrix;
w_r(2,1) = A3;w_r(2,2) = 0;w_r(2,3) = -A1;
w_r(3,1) = -A2;w_r(3,2) = A1;w_r(3,3) = 0;
ew = eye(3)+w_r*s1+w_r*w_r*(1-c1);%指数积公式; exponential product formula;
I = eye(3);
x = cross(w1,v1);
p1 = (I-ew)*x';
T1 = sym(eye(4));
...
w_r(2,1) = A3;w_r(2,2) = 0;w_r(2,3) = -A1;
w_r(3,1) = -A2;w_r(3,2) = A1;w_r(3,3) = 0;
ew = eye(3)+w_r*s2+w_r*w_r*(1-c2);%指数积公式; exponential product formula;
I = eye(3);
x = cross(w2,v2);
p2 = (I-ew)*x';
T2 = sym(eye(4));
...
w_r(2,1) = A3;w_r(2,2) = 0;w_r(2,3) = -A1;
w_r(3,1) = -A2;w_r(3,2) = A1;w_r(3,3) = 0;
ew = eye(3)+w_r*s3+w_r*w_r*(1-c3);%指数积公式;exponential product formula;
I = eye(3);
x = cross(w3,v3);
p3 = (I-ew)*x';
T3 = sym(eye(4));
...
w_r(2,1) = A3;w_r(2,2) = 0;w_r(2,3) = -A1;
w_r(3,1) = -A2;w_r(3,2) = A1;w_r(3,3) = 0;
ew = eye(3)+w_r*s4+w_r*w_r*(1-c4);%指数积公式; exponential product formula;
I = eye(3);
x = cross(w4,v4);
p4 = (I-ew)*x';
T4 = sym(eye(4));
...
w_r(2,1) = A3;w_r(2,2) = 0;w_r(2,3) = -A1;
w_r(3,1) = -A2;w_r(3,2) = A1;w_r(3,3) = 0;
ew = eye(3)+w_r*s5+w_r*w_r*(1-c5);%指数积公式; exponential product formula;
I = eye(3);
x = cross(w5,v5);
p5 = (I-ew)*x';
T5 = sym(eye(4));
...
w_r(2,1) = A3;w_r(2,2) = 0;w_r(2,3) = -A1;
w_r(3,1) = -A2;w_r(3,2) = A1;w_r(3,3) = 0;
ew = eye(3)+w_r*s6+w_r*w_r*(1-c6);%指数积公式;exponential product formula;
I = eye(3);
x = cross(w6,v6);
p6 = (I-ew)*x';
T6 = sym(eye(4));
