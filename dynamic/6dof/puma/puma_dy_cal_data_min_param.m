...
%     U_min_param = zeros(6,57);%每一个轴写成最小集之后，就会少对应的列数；After each axis is written as the smallest set, the corresponding number of columns will be missing;
...
%     tao = U_min_param*fi_min;
        ...
        %% 尝试第二、三、四、五、六关节单独写最小集，其他关节先不动，2、3、4、5、6轴结果ok，Try to write the minimum set separately for the second, third, fourth, fifth, and sixth joints, and the other joints will not move first, and the results of the 2nd, 3rd, 4th, 5th, and 6th axes are OK,
%     YY6 = Ii66yy;M6 = m6;
%     MX6 = m6*Pc666(1); MY6 = m6*Pc666(2);MZ6 = m6*Pc666(3);
%     XX6 = Ii66xx-YY6; XY6 = Ii66xy;XZ6 = Ii66xz;YZ6 = Ii66yz;ZZ6 = Ii66zz;
%     YY5 = Ii55yy;M5 = m5;
...
%     XX2 = Ii22xx-YY2-a3^2*M3;%这个地方是试出来的....不要问我怎么试出来，中间数据各种组合，加上一点灵感，一般人真试不出来。。。谁能不看我公式试出来，我倒立洗头。提示：书上公式已经不顶用了，需要各个列项出来，关键在于M3的系数对不上;
...
%     tao = U_min_param*fi_min;
       ...
       %% 尝试第一、二、三、四、五、六关节单独写最小集，其他关节先不动，1、2、3、4、5、6轴结果ok， Try to write the smallest set of the first, second, third, fourth, fifth, and sixth joints separately, and the other joints will not move first, and the results of the 1, 2, 3, 4, 5, and 6 axes are OK,
    YY6 = Ii66yy;M6 = m6;
    MX6 = m6*Pc666(1); MY6 = m6*Pc666(2);MZ6 = m6*Pc666(3);
    XX6 = Ii66xx-YY6; XY6 = Ii66xy;XZ6 = Ii66xz;YZ6 = Ii66yz;ZZ6 = Ii66zz;
    YY5 = Ii55yy;M5 = m5;
    ...
    XX2 = Ii22xx-YY2-a3^2*M3;%这个地方是试出来的....不要问我怎么试出来，中间数据各种组合，加上一点灵感，一般人真试不出来。。。谁能不看我公式试出来，我倒立洗头。提示：书上公式已经不顶用了，需要各个列项出来，关键在于M3的系数对不上，还有这里的YY3也不在了，是因为我发现YY3的系数跟XX3相反，而在上面公式XX3 = Ii33xx-YY3+YY4+2*d4*MZ4+d4^2*M4也刚好，所以需要去掉YY3;
    ...
    XY2 = Ii22xy;XZ2 = Ii22xz-a3*MZ3;YZ2 = Ii22yz;ZZ2 = Ii22zz+a3^2*M3;     
    ZZ1 = Ii11zz+YY2+2*d2*MZ2+d2^2*M2+YY3 +a3^2*M3+2*d2*MZ3;%这里也是试出来的，根据矩阵的第一行的列项，就是求第一关节的力矩时，某些项需要重新计算，关于为啥第一关节的最小集会涉及到第二关节和第三关节，我觉得可能存在的原因在于theta值，
                                                            %在dh中，第三关节的theta值为-180，从这个地方开始之后，后面的最小集跟书上完全不是一个样，这里感觉书上只是怀疑书上有问题，感觉像是翻转了一下，也就是说三四五六的最小集可以根据书上直接写，
                                                            %但到了第二关节，第一关节，这里明显套不了书上的公式，只能不断的拼凑，比对各个列项的数据，才可以完成最小集的编写；
