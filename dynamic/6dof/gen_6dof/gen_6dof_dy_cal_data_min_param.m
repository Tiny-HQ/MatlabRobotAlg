...
%     tao = U_min_param*fi_min;
    ...
    %% 尝试第五、六关节单独写最小集，其他关节先不动，5、6轴结果ok，
...
%     U_min_param = zeros(6,54);%每一个轴写成最小集之后，就会少对应的列数；%After each axis is written as a minimal set, there will be one fewer corresponding column.
...
%     tao = U_min_param*fi_min;
    ...
    %% 尝试第四、五、六关节单独写最小集，其他关节先不动，4、5、6轴结果ok，
...
%     U_min_param = zeros(6,51);%每一个轴写成最小集之后，就会少对应的列数；%After each axis is written as the minimum set, the corresponding number of columns is missing
...
%     tao = U_min_param*fi_min;
       ...
       %% 尝试第三、四、五、六关节单独写最小集，其他关节先不动，3、4、5、6轴结果ok，
...
%     tao = U_min_param*fi_min;
    ...
    ZZ1 = Ii11zz+YY2+a2^2*M2+YY3+2*d3*MZ3+(a3^2+d3^2)*M3+ a4^2*M4;%% 这里真是绝绝子，我真不服，谁能写出来，我真是服他，跪下的那种服。书上这里完全扯不到边。。谁能想到zz1的最小集里面能写出M3、M4的系数跟参数！！！！！！！！！！
