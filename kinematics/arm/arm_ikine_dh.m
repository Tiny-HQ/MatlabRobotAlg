...
function q = arm_ikine_dh(T,DH,shoulder,elbow,wrist,alpha_theta_option,option)
    ...
    V65 = [0 0 -d6 1]';
    V05 = T*V65;%从第0号坐标系到五号坐标系的位置转换，或者说到第四号坐标系的位置转换； Position conversion from coordinate system 0 to coordinate system 5, or position conversion to coordinate system 4;
    ...
    elseif shoulder == 1
        q(1) = atan2(yp,xp)-atan2(-d3,sqrt(xp^2+yp^2-d3^2));
    elseif shoulder == 0
          %shoulder 奇异,也是前后奇异; shoulder is singularty, but also singularty before and after;
          return;
    end
    ...
    xp1=xp_1*cos(q(1))-a1+yp_1*sin(q(1));%在1号坐标系下，腕关节也就是4号、五号坐标系的位置; In the No. 1 coordinate system, the wrist joint is also the position of the No. 4 and No. 5 coordinate systems;
    yp1=zp-d1;
    ...
    elseif elbow == 1
        ...
        %elbow奇异，也是边界奇异;Elbow is singular, but also borderline singular;
        return;
    end
    if q(2)>pi
        q(2)= q(2)-2*pi;
