...
function tao = cobot_dy_cal_data_min_param(q,dq,ddq)
    ...
    R23 = splitT_R(hrotz(qq3-90));              R32 = R23';
    R34 = splitT_R(hrotz(qq4));     R43 = R34';
    R45 = splitT_R(hrotx(-90)*hrotz(qq5+90));      R54 = R45';
...
%     tao = U_r*fi;
      %% 最小集;
     %% 尝试第六关节单独写最小集，其他关节先不动，结果ok，第六轴结果正确，这里也解释一下，为啥其他轴不对，那是因为写第六关节时，第六关节的相应的列项需要删掉，所以在其他轴还没完成时，才会不对，所以这里只需要关注第六轴力矩即可;
...
%     U_min_param = zeros(6,57);%每一个轴写成最小集之后，就会少对应的列数；
...
%     tao = U_min_param*fi_min;
    ...
    %% 尝试第五、六关节单独写最小集，其他关节先不动，5、6轴结果ok，%Try to write the minimum set separately for the fifth and sixth joints, and the other joints will not move first, and the results of the 5th and 6th axes are OK,
%     YY6 = Ii66yy;M6 = m6;
%     MX6 = m6*Pc666(1); MY6 = m6*Pc666(2);MZ6 = m6*Pc666(3);
%     XX6 = Ii66xx-YY6; XY6 = Ii66xy;XZ6 = Ii66xz;YZ6 = Ii66yz;ZZ6 = Ii66zz;
%     YY5 = Ii55yy;M5 = m5+m6;
...
%     U_min_param = zeros(6,54);%每一个轴写成最小集之后，就会少对应的列数；%After each axis is written as the smallest set, the corresponding number of columns will be missing;
...
%     tao = U_min_param*fi_min;
    ...
    %% 尝试第四、五、六关节单独写最小集，其他关节先不动，4、5、6轴结果ok，%Try to write the minimum set separately for the fourth, fifth, and sixth joints, and the other joints will not move first, and the results of the 4th, 5th, and 6th axes are OK.
%     YY6 = Ii66yy;M6 = m6;
%     MX6 = m6*Pc666(1); MY6 = m6*Pc666(2);MZ6 = m6*Pc666(3);
%     XX6 = Ii66xx-YY6; XY6 = Ii66xy;XZ6 = Ii66xz;YZ6 = Ii66yz;ZZ6 = Ii66zz;
%     YY5 = Ii55yy;M5 = m5+m6;
...
%     MX4 = m4*Pc444(1);MY4 = m4*Pc444(2)+(MZ5+d5*M5);MZ4 = m4*Pc444(3);
%     XX4 = Ii44xx-YY4+YY5+2*d5*MZ5+d5^2*M5;XY4 = Ii44xy;XZ4 = Ii44xz;YZ4 = Ii55yz;ZZ4 = Ii44zz+YY5+2*d5*MZ5+d5^2*M5;
%     fi4_min_param = [MX4 MY4 XX4 XY4 XZ4 YZ4 ZZ4]';
%     fi5_min_param = [MX5 MY5 XX5 XY5 XZ5 YZ5 ZZ5]';
%     fi6_min_param = [MX6 MY6 XX6 XY6 XZ6 YZ6 ZZ6]';
%     fi_min = [fi1;fi2;fi3;fi4_min_param;fi5_min_param;fi6_min_param];
%     U_min_param = zeros(6,51);%每一个轴写成最小集之后，就会少对应的列数；%After each axis is written as the smallest set, the corresponding number of columns will be missing;
...
%     tao = U_min_param*fi_min;
       ...
       %% 尝试第三、四、五、六关节单独写最小集，其他关节先不动，3、4、5、6轴结果ok，%Try to write the minimum set separately for the third, fourth, fifth, and sixth joints, and the other joints will not move first, and the results of the 3rd, 4th, 5th, and 6th axes are OK,
%     YY6 = Ii66yy;M6 = m6;
%     MX6 = m6*Pc666(1); MY6 = m6*Pc666(2);MZ6 = m6*Pc666(3);
%     XX6 = Ii66xx-YY6; XY6 = Ii66xy;XZ6 = Ii66xz;YZ6 = Ii66yz;ZZ6 = Ii66zz;
%     YY5 = Ii55yy;M5 = m5+m6;
...
%     MX3 = m3*Pc333(1)+a4*M4;MY3 = m3*Pc333(2);MZ3 = m3*Pc333(3)+MZ4+d4*M4;
%      XX3 = Ii33xx-YY3-a4^2*M4;XY3 = Ii33xy;XZ3 =
%      Ii33xz-a4*(MZ4+d4*M4);YZ3 = Ii33yz;ZZ3 = Ii33zz+a4^2*M4;%% 凑出来的a4*mx3-a4*d4*xz3-a4^2*xx3+a4^2*zz3，详细查看本文件夹中的ts1.m%Make up a4*mx3-a4*d4*xz3-a4^2*xx3+a4^2*zz3, check the ts1.m in this folder for details
...
%     U_min_param = zeros(6,48);%每一个轴写成最小集之后，就会少对应的列数；%After each axis is written as the smallest set, the corresponding number of columns will be missing;
...
%     tao = U_min_param*fi_min;
       ...
       %% 尝试第二、三、四、五、六关节单独写最小集，其他关节先不动，2、3、4、5、6轴结果ok，
%     YY6 = Ii66yy;M6 = m6;
%     MX6 = m6*Pc666(1); MY6 = m6*Pc666(2);MZ6 = m6*Pc666(3);
%     XX6 = Ii66xx-YY6; XY6 = Ii66xy;XZ6 = Ii66xz;YZ6 = Ii66yz;ZZ6 = Ii66zz;
%     YY5 = Ii55yy;M5 = m5+m6;
...
%     XX4 = Ii44xx-YY4+YY5+2*d5*MZ5+d5^2*M5;XY4 = Ii44xy;XZ4 = Ii44xz;YZ4 = Ii55yz;ZZ4 = Ii44zz+YY5+2*d5*MZ5+d5^2*M5;
%      YY3 = Ii33yy;M3 = m3+m4+m5+m6;
%     MX3 = m3*Pc333(1)+a4*M4;MY3 = m3*Pc333(2);MZ3 = m3*Pc333(3)+d4*M4;%(这里的d4*M4实在太扯了。求关节2的力矩的时候发现M4的系数对不上，简直了。服了，真是服了。也是运气好，把力矩的差值除了一下d4，刚好瞄了一眼MZ3系数，跟MZ3系数一样，不然真要到猴年马月才能试出来)
                                                                        %The d4*M4 here is really. When I found the torque of joint 2, I found that the coefficient of M4 was not matched, which was simple. I took it, I really took it. It's also good luck to remove the difference in torque from d4, and just glance at the MZ3 coefficient, which is the same as the MZ3 coefficient, otherwise it will really take until the year of the monkey to try it out
%      XX3 = Ii33xx-YY3-a4^2*M4;XY3 = Ii33xy;XZ3 = Ii33xz-a4*(MZ4+d4*M4);YZ3 = Ii33yz;ZZ3 = Ii33zz+a4^2*M4;%% 书上的公式，这里已经不给力了，凑出来的a4*mx3-a4*d4*xz3-a4^2*xx3+a4^2*zz3，详细查看本文件夹中的ts1.m 
                                                                                                            %The formula in the book, here is no longer powerful, made up a4*mx3-a4*d4*xz3-a4^2*xx3+a4^2*zz3, check the ts1.m in this folder for details
%      YY2 = Ii22yy;M2 = m2+m3+m4+m5+m6;
%      MX2 = m2*Pc222(1)+a3*M3;MY2 = m2*Pc222(2);MZ2 = m2*Pc222(3)+MZ3;%+MZ3;
%      XX2 = Ii22xx-YY2-a3^2*M3; XY2 = Ii22xy;XZ2 = Ii22xz-a3*MZ3-a3*MZ4;YZ2 = Ii22yz;ZZ2 = Ii22zz+a3^2*M3;   
...
%     U_min_param = zeros(6,45);%每一个轴写成最小集之后，就会少对应的列数；%After each axis is written as the smallest set, the corresponding number of columns will be missing;
...
%     tao = U_min_param*fi_min;
    ...
    %% 尝试第一、二、三、四、五、六关节单独写最小集，其他关节先不动，1、2、3、4、5、6轴结果ok，
    YY6 = Ii66yy;M6 = m6;
    MX6 = m6*Pc666(1); MY6 = m6*Pc666(2);MZ6 = m6*Pc666(3);
    XX6 = Ii66xx-YY6; XY6 = Ii66xy;XZ6 = Ii66xz;YZ6 = Ii66yz;ZZ6 = Ii66zz;
    YY5 = Ii55yy;M5 = m5+m6;
    ...
    XX4 = Ii44xx-YY4+YY5+2*d5*MZ5+d5^2*M5;XY4 = Ii44xy;XZ4 = Ii44xz;YZ4 = Ii55yz;ZZ4 = Ii44zz+YY5+2*d5*MZ5+d5^2*M5;
     YY3 = Ii33yy;M3 = m3+m4+m5+m6;
    MX3 = m3*Pc333(1)+a4*M4;MY3 = m3*Pc333(2);MZ3 = m3*Pc333(3)+d4*M4;%(搞了一个星期，这里的d4*M4实在太扯了。求关节2的力矩的时候发现M4的系数对不上，简直了。服了，真是服了。也是运气好，把力矩的差值除了一下d4，刚好瞄了一眼MZ3系数，跟MZ3系数一样，不然真要到猴年马月才能试出来)
                                                                      %I've been doing it for a week, and the d4*M4 here is too. When I found the torque of joint 2, I found that the coefficient of M4 was not matched, which was simple. I took it, I really took it. It's also good luck to remove the difference in torque from d4, and just glance at the MZ3 coefficient, which is the same as the MZ3 coefficient, otherwise it will really take until the year of the monkey to try it out
    XX3 = Ii33xx-YY3-a4^2*M4;XY3 = Ii33xy;XZ3 = Ii33xz-a4*(MZ4+d4*M4);YZ3 = Ii33yz;ZZ3 = Ii33zz+a4^2*M4;%% 书上的公式，这里已经不给力了，凑出来的a4*mx3-a4*d4*xz3-a4^2*xx3+a4^2*zz3，详细查看本文件夹中的ts1.m  
     YY2 = Ii22yy;M2 = m2+m3+m4+m5+m6;
     MX2 = m2*Pc222(1)+a3*M3;MY2 = m2*Pc222(2);MZ2 = m2*Pc222(3)+MZ3;%+MZ3;
     XX2 = Ii22xx-YY2-a3^2*M3; XY2 = Ii22xy;XZ2 = Ii22xz-a3*MZ3-a3*MZ4;YZ2 = Ii22yz;ZZ2 = Ii22zz+a3^2*M3;  
     ZZ1 = Ii11zz+YY2 +a3^2*M3+YY3+YY4+2*d4*MZ4+(a4^2+d4^2)*M4;%@#$^&$#*%$^%^&%&^&)&*&_&_+*(*(&*^&%^#$%@#!@#这就是我想说的话。%That's what I want to say.
     fi1_min_param = [ZZ1]';
    fi2_min_param = [MX2 MY2 XX2 XY2 XZ2 YZ2 ZZ2]';
    fi3_min_param = [MX3 MY3 XX3 XY3 XZ3 YZ3 ZZ3]';
    fi4_min_param = [MX4 MY4 XX4 XY4 XZ4 YZ4 ZZ4]';
    ...
    U_min_param = zeros(6,36);%每一个轴写成最小集之后，就会少对应的列数；%After each axis is written as the smallest set, the corresponding number of columns will be missing;
