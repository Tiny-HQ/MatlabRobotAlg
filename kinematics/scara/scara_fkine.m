...
%ori表示左手还是右手; ori indicates whether it is left or right;
%此处的scara是桌面式;不包含吊装; The scara here is desktop; Lifting is not included;
function [T,ori] = scara_fkine(q,DH,q_option,a_t_option)
    ...
    %q2的范围是-180到+180 Q2 ranges from -180 to +180
    if abs(q(2))<eps
        ori = 0;
    elseif q(2)>0
        ori = 1;
