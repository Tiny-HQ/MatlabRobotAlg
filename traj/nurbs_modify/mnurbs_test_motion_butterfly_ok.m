...
for j = 1:length(u)
    ...
    [Cu(j,1),dCu(j,1),ddCu(j,1),Cu(j,2),dCu(j,2),ddCu(j,2),Cu(j,3),dCu(j,3),ddCu(j,3)] = mnurbs_cal_ddcu_derivative(u(j),k,u_vec,P(:,1),w',P(:,2),w',P(:,3),w');
...
for i = 1:length(pp)-1
    s(i) = mnurbs_simpson_cal_length(u(pp(i)),u(pp(i+1)),u_vec,k,P(:,1),w',P(:,2),w',P(:,3),w');
end
...
for i = 1:length(v)-2
    while (1)
        [t(i,:),j,error] = s_curve_all_parameter_cal_para(ss(seg-i+1),ss(seg-i),v(seg-i+1),v(seg-i),V_max,A_max,J_max);
        if(error == 1)
           v(seg-i) = v(seg-i)/2.0;% velocity decrease to satisfy the need.
        else
            break;
        end
    end 
...
for i = 1:total-1
     ...
     [Cu1(i,1), dcux, ddcux,Cu1(i,2), dcuy, ddcuy,Cu1(i,3), dcuz, ddcuz] =  mnurbs_cal_ddcu_derivative(u_t,k,u_vec,P(:,1),w',P(:,2),w',P(:,3),w');
     ds = s_t(i+1)-s_t(i);
     dcu = [dcux dcuy dcuz];
